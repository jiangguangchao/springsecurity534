...
public class DatabaseMetaDateApplication {
    ...
    private void getDatabaseMetaData() {
        try {
            if (dbMetaData == null) {
                ...
                String url = "jdbc:mysql://localhost:3306/jgctest";
                String user = "root";
                String password = "jgc";
     ...
     * 获得表或视图中的所有列信息
     */
    public void getTableColumns(String catalog, String schemaName, String tableName) {

        try{

            ResultSet rs = dbMetaData.getColumns(catalog, schemaName, tableName, "%");
            while (rs.next()){
...
//		metaData.getAllSchemas();
		metaData.getTableColumns("jgctest", null,"t_user");
